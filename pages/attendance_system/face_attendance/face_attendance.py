...
class FaceAttendanceScreen(BaseScreen):
    ...
    employee_code = StringProperty("")
    verification_mode = BooleanProperty(False)
    
    # Django API Configuration
    API_BASE_URL = "http://192.168.1.47:8080/api/attendance"
    EMPLOYEE_API_BASE = "http://192.168.1.47:8080/api/employees"
    ...
    def toggle_verification_mode(self):
        ...
        if self.verification_mode:
            ...
            self.show_api_status("📋 Manual Mode", "success")
        else:
            if self._auto_recognize_event:
                try:
                    self._auto_recognize_event.cancel()
            ...
            self.show_api_status("🤖 Auto Mode", "success")
    ...
    def on_api_error(self, request, error, name, action):
        ...
        self.show_api_status(f"Connection error", "error")
        ...
        if not self.verification_mode:
            if self._auto_recognize_event:
                try:
                    ...
                    pass
            self._auto_recognize_event = Clock.schedule_interval(self.capture_for_recognition, 3.0)
        ...
        try:
            while not self.recognition_stopped:
                try:
                    ...
                    result = self.face_system.register_face_from_frame(frame, "temp")
                    
                    if result['success']:
                        embedding = result['embedding'].tolist()
                        
                        # Send to backend for recognition
                        Clock.schedule_once(lambda dt: self._recognize_face_backend(embedding), 0)
    ...
    def _recognize_face_backend(self, embedding):
        """Call backend API to recognize face"""
        if self.processing or self.verification_mode:
            return
        ...
        try:
            ...
            response = requests.post(
                f"{self.EMPLOYEE_API_BASE}/recognize_face/",
                json={"face_encoding": embedding},
                timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('recognized'):
                    ...
                    # Check cooldown
                    with self.data_lock:
                        if employee_name not in self.last_recognized or \
                           (current_time - self.last_recognized[employee_name]).seconds > 60:
                            
                            self.update_welcome_message(employee_name)
                            print(f"✓ Recognized: {employee_name} | Distance: {distance:.4f}")
                            
                            # Hit Django attendance API
                            self.hit_django_api(employee_name, self.current_time_type)
                            
                            self.last_recognized[employee_name] = current_time
            
            self.processing = False
            
        except Exception as e:
            print(f"Backend recognition error: {e}")
            self.processing = False
