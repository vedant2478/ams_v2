...
def get_battery_percentage(lib_battery):
    return str(lib_battery.getBatteryPercentage())
...
def post_cabinet_status(session, lib_battery, lib_KeyboxLock):
    is_activity = (
    ...
    try:
        ...
        BATTERY_CHARGE_PC = get_battery_percentage(lib_battery)
        cabinet_status_data = {
            ...
            if table_name == "AMSC_CONFIG_ACTIVITIES":
                ...
                for record in records:
                    ...
                    activity = (
                        ...
                        .filter(AMS_Activities.id == record["ACTIVITY_ID"])
                        .first()
                    )
                    if activity:
                ...
                for record in records:
                    ...
                    user = (
                        session.query(AMS_Users)
                        .filter(AMS_Users.id == record["USER_ID"])
                        .first()
                    )
                    if user:
                ...
                for record in records:
                    ...
                    event = (
                        ...
                        .filter(AMS_Events.id == record["ID"])
                        .first()
                    )
                    if event:
                ...
                for record in records:
                    ...
                    key = (
                        ...
                        .filter(AMS_Keys.id == record["KEY_ID"])
                        .first()
                    )
                    if key:
...
if __name__ == "__main__":
    ...
    schedule.every(2).minutes.do(
        post_cabinet_status, session, lib_battery, lib_KeyboxLock
    )
