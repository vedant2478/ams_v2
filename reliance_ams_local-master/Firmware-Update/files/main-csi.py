    ...
    if login_option_str == "1":
        ...
        if recordset:
            for rows in recordset:
                ...
                # verify fingerprint-check with user_template and reference_file:
...
def get_activity_code(lib_display, lib_keypad, FD_KEYPAD):
    ...
    while act_char_count < 2:
        key = lib_keypad.keypadHandler(FD_KEYPAD)
        key_str = str(KEY_DICT[str(key)])
        # print("Key Count : " + str(act_char_count))
        if key is not None:
            if key_str.isnumeric():
                # print("Number entered : " + key_str + "\n")
...
def main():
    ...
    # print("Testing buzzer: its ON")
    # lib_Buzzer.setBuzzerOn()
    # sleep(3)
    # lib_Buzzer.setBuzzerOff()
    # print("Buzzer now OFF")
    ...
    # timesec = 0
    # while timesec < 30:
    #     x = lib_KeyboxLock.getDoorSensorStatus1()
    #     sleep(1)
    #     print("Door status 1: " + str(x))
    #     timesec += 1
    #     # sleep(2)
    #     # print("Door status 2: " + str(lib_KeyboxLock.getDoorSensorStatus2()))
    #
    # print("Now, testing door solenoid lock....UN-LOCKING NOW..")
    # ams_can.lib_door_lock.setKeyBoxLock(ams_can.Lock_FD, 1, 1, 1)
    # sleep(5)
    # print("\nLOCKING NOW..")
    ...
    # print("Testing Card reader...please swipe card: ")
    #
    # while True:
    #     card_info = lib_battery.getCardDetails()
    #     print("\nCard info from library: " + str(card_info))
    #     sleep(2)
    #
    #
    ...
    try:
        lib_display.displayString("CHECKING UPDATES".encode("utf-8"), 1)
    ...
    if cabinet:
        ...
        # if len(ams_can.key_lists) < 2:
        #     both_strip_initialized = False
        #     ams_can.cleanup()
        #     lib_display.displayString("Starting Keyrack".encode('utf-8'), 1)
        #     lib_display.displayString("Please wait.....".encode('utf-8'), 2)
        #     counter_ams_cycle = 0
        #     while not both_strip_initialized:
        #         counter_ams_cycle += 1
        #         ams_can = AMS_CAN()
        #         sleep(1)
        #         if len(ams_can.key_lists) == 2:
        #             both_strip_initialized = True
        #             lib_display.displayString("Done            ".encode('utf-8'), 2)
        #             break
        #         elif counter_ams_cycle >5:
        #             # ams_can.cleanup()
        #             both_strip_initialized = False
        #             lib_display.displayString("Pls re-start... ".encode('utf-8'), 2)
        #             break
        #         else:
        #             ams_can.cleanup()
        #             sleep(1)
                ...
                if key_str == "ENTER":
                            ...
                            try:
                                ...
                                if dic_result["ResultCode"] == ACTIVITY_ALLOWED:
                                    ...
                                    if IS_KEY_IN_WRONG_SLOT:
                                        sleep(1)
                                        lib_display.displayString(
                                            "Key in Wrong Pos".encode("utf-8"), 1
                                        )
                    ...
                    if not (pin_entered == ""):
                        ...
                        if user_auth["ResultCode"] == AUTH_RESULT_SUCCESS:
                            ...
                            if roleId == 1:
                                print("############   Step 21  ###########")
                                lib_display.displayString(
                                    "1. Reg. Card    ".encode("utf-8"), 1
                                )
                                lib_display.displayString(
                                    "2. Reg. Finger".encode("utf-8"), 2
                                )
                                ...
                                if key_str == "3":
                                    ...
                                    if door_status == 1:
                                        for keylistid in ams_can.key_lists:
                                            ...
                                            ams_can.set_all_LED_OFF(keylistid)
                                        lib_display.displayString(
                                            "WELCOME AMS V1.1".encode("utf-8"), 1
                                        )
                                        continue

                                    lib_display.displayString(
                                        "Insert all keys ".encode("utf-8"), 1
                                    )
                                    lib_display.displayString(
                                        "and press ENTER ".encode("utf-8"), 2
                                    )
                                    key = lib_keypad.keypadHandler(FD_KEYPAD)
                                    key_str = str(KEY_DICT[str(key)])
                                    if key_str == "ENTER":
                                        ...
                                        lib_display.displayString(
                                            "Scan in progress".encode("utf-8"), 1
                                        )
                                        lib_display.displayString(
                                            "Pls Wait...     ".encode("utf-8"), 2
                                        )
                                    ...
                                    lib_display.displayClear()
                                    lib_display.displayString(
                                        "Enter User PIN".encode("utf-8"), 1
                                    )
                                    pin_entered = login_using_PIN(
                                        lib_display, lib_keypad, FD_KEYPAD
                                    )
                                    lib_display.displayString(
                                        "Validating PIN..".encode("utf-8"), 2
                                    )
                                    ...
                                    if not (pin_entered == ""):
                                        ...
                                        if (
                                            ...
                                            lib_display.displayClear()
                                            lib_display.displayString(
                                                "Swipe User Card ".encode("utf-8"), 1
                                            )
                                            timer_sec = 0
                                            card_no = None
                                            ...
                                            if card_no_updated:
                                                lib_display.displayClear()
                                                lib_display.displayString(
                                                    "Card Registered".encode("utf-8"), 1
                                                )
                                    ...
                                    if not (pin_entered == ""):
                                        ...
                                        if (
                                            ...
                                            lib_display.displayClear()
                                            lib_display.displayString(
                                                "Scan finger now ".encode("utf-8"), 1
                                            )
                                            ...
                                            # Check 'returncode' value from 'return_val'
                                            # 0 value mean success, in case of non-zero value, try again
                                            # returnstr = 'returncode=0'
                                            # if returnstr in str(return_val):
                                            #     print("Success")
                                            # else:
                                            #     print("Try again")
                                            ...
                                            lib_display.displayString(
                                                "Scan complete...".encode("utf-8"), 1
                                            )
                                            lib_display.displayString(
                                                "Saving, pls wait".encode("utf-8"), 2
                                            )
                                            enroll_data = subprocess.run(
                                                [
                                                    "/home/ams-core/FCEnrollFingerFromImageCPP",
                                            ...
                                            session.commit()
                                            lib_display.displayString(
                                                "FP registration ".encode("utf-8"), 1
                                            )
                                            lib_display.displayString(
                                                "done, pls wait  ".encode("utf-8"), 2
                                            )
                                            sleep(2)
                                            continue
                                elif key_str == "F1":
                    ...
                    if not (pin_entered == ""):
                        ...
                        if user_auth["ResultCode"] == AUTH_RESULT_SUCCESS:
                            ...
                            if roleId == 1:
                                lib_display.displayString(
                                    "Take/Return Keys".encode("utf-8"), 2
                                )
                                ams_can.unlock_all_positions(1)
                                ams_can.set_all_LED_ON(1, False)
